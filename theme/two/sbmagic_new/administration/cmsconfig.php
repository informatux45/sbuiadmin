...
switch($action) {
	...
	case "plugins":
		...
		// --------------------------------
		$tab_jquery = array();
		$tab_jquery[0]['text']    = 'JQUERY (Latest)';
		$tab_jquery[0]['name']    = 'jquery';
		$tab_jquery[0]['checked'] = (in_array("jquery", $plugins)) ? '1' : '0';
		$sbform->addCheckbox('Activation du plugin', $tab_jquery, '', false, '<br />');
		$sbform->addAnything(sbHowToPlugins('Comment utiliser le plugin JQUERY', 'jquery/howto.html', 'Zero'));
		// --------------------------------
		// --- Plugin LIGHTBOX
		// --------------------------------
		$tab_lightbox = array();
		$tab_lightbox[0]['text']    = 'LIGHTBOX';
		$tab_lightbox[0]['name']    = 'lightbox';
		$tab_lightbox[0]['checked'] = (in_array("lightbox", $plugins)) ? '1' : '0';
		$sbform->addCheckbox('Activation du plugin', $tab_lightbox, '', false, '<br />');
		$sbform->addAnything(sbHowToPlugins('Comment utiliser le plugin LIGHTBOX', 'lightbox/howto.html', 'One'));
		// --------------------------------
		// --- Plugin FANCYBOX
		// --------------------------------
		$tab_fancybox = array();
		$tab_fancybox[0]['text']    = 'FANCYBOX';
		$tab_fancybox[0]['name']    = 'fancybox';
		$tab_fancybox[0]['checked'] = (in_array("fancybox", $plugins)) ? '1' : '0';
		$sbform->addCheckbox('Activation du plugin', $tab_fancybox, '', false, '<br />');
		$sbform->addAnything(sbHowToPlugins('Comment utiliser le plugin FANCYBOX', 'fancybox/howto.html', 'Two'));
		// --------------------------------
		// --- Plugin CHECKBOXCSS
		// --------------------------------
		$tab_checkboxcss = array();
		$tab_checkboxcss[0]['text']    = 'CHECKBOXCSS';
		$tab_checkboxcss[0]['name']    = 'checkboxcss';
		$tab_checkboxcss[0]['checked'] = (in_array("checkboxcss", $plugins)) ? '1' : '0';
		$sbform->addCheckbox('Activation du plugin', $tab_checkboxcss, '', false, '<br />');
		$sbform->addAnything(sbHowToPlugins('Comment utiliser le plugin CHECKBOXCSS', 'checkboxcss/howto.html', 'Three'));
		// --------------------------------
		// --- Plugin APPEAR / DISAPPEAR
		// --------------------------------
		$tab_appear = array();
		$tab_appear[0]['text']    = 'APPEAR / DISAPPEAR';
		$tab_appear[0]['name']    = 'appear';
		$tab_appear[0]['checked'] = (in_array("appear", $plugins)) ? '1' : '0';
		$sbform->addCheckbox('Activation du plugin', $tab_appear, '', false, '<br />');
		$sbform->addAnything(sbHowToPlugins('Comment utiliser le plugin APPEAR / DISAPPEAR', 'appear/howto.html', 'Four'));
		// --------------------------------
		// --- Hiddens / Buttons
		// --------------------------------	
		$sbform->addInput('hidden', '', array('name' => 'form_submit', 'value' => "$formName"));
