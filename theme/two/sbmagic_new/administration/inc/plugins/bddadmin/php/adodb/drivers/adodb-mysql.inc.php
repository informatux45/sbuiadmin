		...
		while (!$rs->EOF){
			...
			if (preg_match("/^(.+)\((\d+),(\d+)/", $type, $query_array)) {
				...
				*/	
				//
				// Works, could be slow
				if (preg_match("/'(.*)'/", $query_array[2], $enums))
					$arr = explode("','", $enums[1]);
				else
					$arr = Array();
				//					
				/*
				// Does not work in case of empty first argument (given: '', '1', '2', the 'trim' discards both initial ')
				$arr = explode("','",trim($query_array[2], "'"));
				*/
				//
				$fld->enums = $arr;
