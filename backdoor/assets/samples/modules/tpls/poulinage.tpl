...
{* -------------- *}
	...
	{* ---------------- End Headers --------------- *}
                ...
                <div class="col-lg-12">
                    <div class="panel panel-{if $all}primary{else}default{/if}">
                        <div class="panel-heading">
                            <i class="fa fa-star-half-empty fa-fw"></i> Toutes les juments
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
							{if $all}
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-poulinage">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
										{foreach from=$all item=foreachall}
											<tr class="{cycle values="odd,even"} gradeX">
												...
												<td>{$foreachall.dob|unescape:"htmlall"}</td>
												<td>{$foreachall.country|unescape:"htmlall"}</td>
												<td>{$foreachall.sire|unescape:"htmlall"}</td>
												<td>
		...
		$(document).ready(function() {
			// Your own code
			$('#dataTables-poulinage').DataTable({
					responsive: true
			});
		});
		</script>
