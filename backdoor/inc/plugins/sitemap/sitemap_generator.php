...
function Scan ($url)
{
    ...
    {   
        ...
        echo "Redirected to: $url" . NLBR;
    }
    // Handle other codes than 200
    else if (strpos ($headers[0], "200") == false)
    {
        $url = $headers["Location"];
        echo "Skip HTTP code $headers[0]: $url" . NLBR;
        return false;
    }
    ...
    if ($content_type != "text/html") 
    {
        ...
        {
            echo "Info: Ignoring empty Content-Type." . NLBR;
        }
        else
        {
            if ($content_type == "")
            {
                echo "Info: Content-Type is not sent by the web server. Change " .
                     ...
                     "to scan those pages too." . NLBR;
            }
            else
            {
                echo "Info: $url is not a website: $content[0]" . NLBR;
            }
            return false;
        }
    }
    ...
    if (!$pf)
    {
        echo "Cannot create " . OUTPUT_FILE . "!" . NLBR;
        return;
    }
    ...
    echo "Done. " . OUTPUT_FILE . " created." . NLBR;
	
	// Move file
	$rename = rename(OUTPUT_FILE, SB_PATH . OUTPUT_FILE);
	if ($rename)
		echo "All Done.";
	else
		echo "!!! Error Moving file !!!" . NLBR . "inc/plugins/" . basename(__DIR__) . '/' . OUTPUT_FILE . ' ==> ' . SB_PATH . OUTPUT_FILE;
?>